...
public class APIExceptionHandler {
    ...
    public final ResponseEntity<Object> persistException(Exception e){
        APIException exception = new APIException("Error persisting data", HttpStatus.BAD_REQUEST, LocalDateTime.now());
    ...
    public final ResponseEntity<Object> badRequest(Exception e){
        APIException exception = new APIException("User already exists", HttpStatus.BAD_REQUEST, LocalDateTime.now());
        logger.debug(e.getMessage());
        return new ResponseEntity<>(exception, HttpStatus.BAD_REQUEST);
    }
    @ExceptionHandler(NotFoundReqException.class)
    public final ResponseEntity<Object> entityNotFound(Exception e){
        APIException exception = new APIException("Data not found", HttpStatus.NOT_FOUND, LocalDateTime.now());
        logger.debug(e.getMessage());
    ...
    public final ResponseEntity<Object> genericException(Exception e){
        APIException exception = new APIException(HttpStatus.INTERNAL_SERVER_ERROR.getReasonPhrase(),  HttpStatus.INTERNAL_SERVER_ERROR, LocalDateTime.now());
        logger.debug(e.getMessage());
